...
## 2026-07-21 ~ 2026-07-22（补记）

**审核整改：静默注册（`667e3bb`）**
- 审核被拒（进入即强制授权头像/昵称）。移除登录闸门与 login 路由：首次进入以 openid 静默注册，`ensureUser` 哈希生成默认昵称（如「安然刺猬」）/首字头像/颜色，全功能免授权可用；头像昵称在「我的」用户主动改（点头像换头像走 chooseAvatar 覆盖层 button）。join/注销/清空流程同步调整。
- 修 `-504003` 云函数超时：reset 后全新环境 `book.create` 串行 ~83 次 DB 调用必超 3 秒——`ensureCollections` 并行、默认分类批量数组插入（68 次→4 次），config.json `timeout: 20` 兜底。

**结算币种灵活化（`667e3bb` + 后续）**
- 结算页金额跟随「账本 × 用户」展示币种（此前直出基准币，ISK 账本显示 €）；每笔转账可选结算币种（A 收欧元、B 给人民币），按 from|to 记在 books.settleCur 双方同口径；标记结清快照 currency/amountFx 历史不漂移；合计折合展示币种；净额列与垫付/应摊同精度；移除页内 FAB。
- `getRate` 改 CNY 枢轴换算，修复非 CNY 基准账本记账/汇率全废的 blocker。

**多币种精度修复（1000 ISK 显示 1000.78/汇总 1000.07）**
- 三层：①展示币种==原始币种直接用原始金额（recToDisplay 短路）；②`amountConverted` 固化精度 round2→round6；③汇率按有效数字保精度（`toPrecision(12)`，round6 对 ISK→EUR 只剩 4 位有效数字）——列表/月汇总/图表全口径归正。存量旧记录需重新保存一次。

**分账账本展示**
- 首页支出行显示付款人头像 +「XX 付款 · N 人分摊」（名字可省略、后缀不截断；付款人缺资料用其名字首字兜底，不借记录人身份）；详情页新增「分摊」行（成员头像叠排 + 均摊各多少）。
- 金额排版：全站币种符号与数字间加窄空格（`-¥ 1,047.96`、`kr 1,000.00`）。

**其他**
- 记一笔金额框：受控 focus 加 bindblur 复位（收键盘后再点不弹的经典失同步）。
- Learning.md 机制落地：根目录教训库（回填 13 条）+ CLAUDE.md 约定 + UserPromptSubmit hook 每轮提醒；本两日新增 8 条教训。
- git：`667e3bb` 已提交（审核整改+结算币种+枢轴汇率）。
...
**AI 语音输入（腾讯云 ASR）**
- 主体为个人无法添加官方「同声传译」插件（类目限制），改走方案 B：录音（mp3 16k ≤60s）→ 云存储中转（callFunction 传参有大小上限）→ 云函数**手写 TC3-HMAC-SHA256 签名**直调腾讯云「一句话识别」（零第三方依赖，免云端装依赖）。密钥存云函数环境变量 `TENCENT_SECRET_ID/KEY`；音频识别完即删（finally，不留存）。
- 交互：麦克风按钮微信风格**内嵌输入框右侧**，有文字即隐藏；按住说话 + 声浪浮层 + 上滑取消（变红）；识别文本填输入框不自动发送。授权流程（scope.record + 拒绝引导 openSetting）、<0.6s 拦截、额度耗尽（NoFreeAmount/Arrears…）/鉴权异常/失败三级错误映射，全部降级到手动输入。
- 计费护栏：腾讯云后付费默认关闭（不点「开通后付费」+ 不买资源包 = 花费上限 0，免费包用完服务自停）。跨境不受影响：ASR 请求由境内云函数发出，用户在欧洲只多录音上传延迟。
- mp 后台配套（用户操作）：隐私保护指引勾选麦克风（用途：语音输入转文字）、5.2 告知方式填「弹窗提示」。

**收入默认分类扩充**
- 2 类 → **8 类**与支出对齐：工资薪酬/兼职副业/投资理财/租金经营/人情往来/报销退款/奖励中奖/其他收入（含二级）；`parseNL` 收入关键词与新分类同步（兼职/稿费/礼金/压岁钱/收租/基金/股票/加班/中奖…）；seed 演示收入分类同步为 5 类。分类挂账本：仅新建账本/重载演示数据生效，存量账本不动。

**统计钻取三修**
- 「收入点进去显示支出」根因：上一日的 `recordType` 撞名修复只在工作区、**云函数未重部署**，线上旧代码筛选整体失效——代码无须再改，重部署即好。
- 月度/累计卡「结余」行可钻取（不带收支筛选，收入+支出合看）；柱状卡明细入口条文案按语义分段（日期/收/支 flex-wrap），杜绝金额中间截断。
- records 钻取页导航+汇总条 `sticky-header` 吸顶（复用首页方案）。

**反馈工单体验**
- feedback-new 邮箱格式错误由 toast 改为**输入框标红 + field-err**（复用 onboarding 空账本名模式，输入合法即消除）。
- 客服视角：标题「反馈工单（客服）」精简为「反馈工单 + 客服视角副行」（长标题挤压 + 按钮）；「用户工单」分区空态常驻提示（此前队列空则整段隐藏，管理员误以为无处理入口）。seed 新增 4 条工单样例（三状态 + 我的未读回复红点，假用户提交）。

**设置 / 隐私 / 其他**
- 新增 `pages/privacy` 隐私说明页（数据存储/可见性边界/AI 使用范围/汇率/导出与反馈，语音一句「转文字即删」）；设置页隐私行改跳转。
- 版本号去掉假数据「v1.0 · P1」：正式版取 `wx.getAccountInfoSync` 真实版本，体验版/开发版按环境标注；产品文案中所有「P1 …」内部分期话术清除。
- 记账页分账方式删除与均摊功能重复的「按人指定」（P1 无按人金额，两者数据完全一致），旧记录 `by` 模式编辑时归一化为均摊。
- 设置页个人头副行拆两行（已加入 N 个账本 / 默认「xxx」），超长省略。
- dev「清空所有数据」升级：删库前收集头像/记录图/反馈截图/AI 收据/导出台账的云存储 fileID 一并删除；导出文件新增 `files` 集合台账（下载路径的导出文件此前无引用、无法回收）。确认「载入演示数据」自带先清后写，无须手动先清除。
- git 提交并推送 `240e225`（应用户指令，46 文件 +1558/−336）。

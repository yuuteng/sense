...
4. **云函数环境变量**（云开发控制台 → 云函数 `api` → 配置）：
   ...
   | 变量 | 用途 | 必需 |
   |---|---|---|
   | `FEEDBACK_OWNER` | 应用 owner 的 openid（逗号分隔可多个）。既是反馈工单客服 owner，也是**全部开发者工具（刷新汇率/演示数据/清理测试渠道/清空所有数据）的唯一权限源**——配置后这些功能只认此身份，与 `APP_ENV` 无关 | 建议尽早配 |
   | `APP_ENV` | 兜底：仅在**未配** `FEEDBACK_OWNER` 时生效，`dev` 开启开发者工具（早期开发/新环境用） | 开发时 |
   ...
   | `AI_FREE_QUOTA` | AI 限额开关：**缺省/0 = 不限次数、AI 页隐藏额度文案（当前默认）**；配 `50` 等正数 = 每用户免费 N 次真实模型调用，用尽后问答/收据识别降级并提示。改值即时生效，无需重新部署；用量始终在 `users.aiUsage` 计数，关着也可观察消耗决定何时开启 | 额度扛不住时 |
...
> **安全**：开发者工具以 `FEEDBACK_OWNER` 身份（服务端 openid 比对，不可伪造）为唯一权限源，`APP_ENV=dev` 仅作未配 owner 时的兜底。默认（无任何配置）全部禁用，公开仓库无法被利用。所有密钥只放云函数环境变量，切勿写进代码。
